...
import { Team, users, teamMembers } from '@/lib/db/schema';
import {
...
export async function handleSubscriptionChange(
  ...
  if (status === 'active' || status === 'trialing') {
    ...
    const teamUsersResult = await db
      .select({ userId: teamMembers.userId })
      .from(teamMembers)
      .where(eq(teamMembers.teamId, team.id));

    const teamUserIds = teamUsersResult.map(result => result.userId);
    ...
    // Revert all users in the team to the free plan
    if (teamUserIds.length > 0) {
      await db
        .update(users)
        .set({ plan: 'free' })
